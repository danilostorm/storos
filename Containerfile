...
COPY src/storos_execution_admission.py /usr/libexec/storos/storos_execution_admission.py
COPY src/storos_execution_authority.py /usr/libexec/storos/storos_execution_authority.py
...
RUN chmod 0755 /usr/bin/storosctl \
    && PYTHONPATH=/usr/libexec/storos python3 -c "from storos_execution_contract import execution_contract,backend_capabilities,authorization_decision; from storos_execution_admission import DenyOnlySimulationAdapter; c=execution_contract(); b=backend_capabilities(); a=DenyOnlySimulationAdapter(); assert c['executable'] is False and c['worker_available'] is False and c['feature_gate_may_enable_write'] is False; assert b['mutating_available'] is False and all(v['supported'] is False for v in b['actions'].values()); assert authorization_decision('image-smoke','start_vm','11111111-2222-3333-4444-555555555555')['granted'] is False; assert not hasattr(a, 'apply'); assert a.describe()['mutating_available'] is False and a.describe()['apply_method_available'] is False; from storos_execution_authority import DenyOnlyCapabilityProvider,identity_record,authenticate_local_identity,authorize_execution_action; p=DenyOnlyCapabilityProvider(); i=identity_record('image.local',1000,1000,['vm:action:start_vm'],['11111111-2222-3333-4444-555555555555']); ctx=authenticate_local_identity(1000,1000,{'schema_version':1,'generation':1,'identities':[i]}); assert authorize_execution_action(ctx,'start_vm','11111111-2222-3333-4444-555555555555')['granted'] is True; assert not hasattr(p, 'apply'); assert p.describe()['mutating_available'] is False and p.negotiate('start_vm')['eligible'] is False" \
